				...
				<div class="result-section" v-if="state.predictionResult.label">
					<el-card class="result-card">
						...
						<div class="result-content">
							<div class="result-images" v-if="imageUrl || resultImageUrl">
								<div class="result-image-item" v-if="imageUrl">
									<div class="result-image-title">原始图片</div>
									<img :src="imageUrl" class="result-preview-image" alt="原始图片" />
								</div>
								<div class="result-image-item" v-if="resultImageUrl">
									<div class="result-image-title">检测结果图片</div>
									<img :src="resultImageUrl" class="result-preview-image" alt="检测结果图片" />
								</div>
							</div>
...
import { reactive, ref, onMounted, onBeforeUnmount, onDeactivated, computed, nextTick } from 'vue';
...
/**
 * 与视频页思路一致：尽量不在上传前做重活；仅在「大图/大图幅」时缩放到 YOLO 常用量级，减轻上传与后端解码耗时。
 * - 小体积 JPEG 直接原图上传
 * - 优先 createImageBitmap（避免 readAsDataURL 整文件进内存）
 * - 回退用 blob URL + Image，仍优于 DataURL
 */
const IMG_MAX_EDGE = 960;
const JPEG_QUALITY = 0.78;
const SMALL_JPEG_BYTES = 750 * 1024;

const compressImage = (rawFile: File): Promise<File> => {
	return new Promise((resolve, reject) => {
		const ext = rawFile.name.split('.').pop() || 'jpg';
		const toJpegFile = (blob: Blob) =>
			new File([blob], rawFile.name.replace(/\.[^.]+$/, '.jpg'), {
				type: 'image/jpeg',
		...
		const drawScaled = (source: CanvasImageSource, w: number, h: number) => {
			...
			if (edge <= IMG_MAX_EDGE) {
				resolve(rawFile);
				return;
			}
			const scale = IMG_MAX_EDGE / edge;
		...
		// 小 JPEG 与视频页一样：不做前端缩放
		if (rawFile.size <= SMALL_JPEG_BYTES && /jpeg|jpg/i.test(rawFile.type)) {
			resolve(rawFile);
			return;
		}

		if (typeof createImageBitmap === 'function') {
			createImageBitmap(rawFile)
				.then((bitmap) => {
					try {
						drawScaled(bitmap, bitmap.width, bitmap.height);
					} finally {
						bitmap.close();
					}
				})
				.catch(() => loadWithObjectUrl());
			return;
		}

		loadWithObjectUrl();

		function loadWithObjectUrl() {
			const url = URL.createObjectURL(rawFile);
			const img = new Image();
			img.onload = () => {
				URL.revokeObjectURL(url);
				drawScaled(img, img.naturalWidth || img.width, img.naturalHeight || img.height);
			};
			img.onerror = () => {
				URL.revokeObjectURL(url);
...
const beforeImageUpload: UploadProps['beforeUpload'] = async (rawFile: any) => {
...
/** 离开页面或发起新检测时递增，避免异步回调在卸载后仍改 state 导致路由异常 */
let predictRenderSeq = 0;
...
const upData = () => {
	...
	request
		...
		.then((res: any) => {
			if (seq !== predictRenderSeq) return;
			...
			try {
				if (payload.outImg) {
					resultImageUrl.value = resolveFileUrl(payload.outImg);
				} else {
					resultImageUrl.value = '';
				}
				...
				// 先结束 loading、让结果区立刻可渲染；Markdown、表格数据与写库放到下一帧，不阻塞界面
				state.predictionResult.suggestion = '';
				state.loading = false;
				void nextTick(() => {
					if (seq !== predictRenderSeq) return;
					try {
						const sug = String(payload.suggestion ?? '').trim();
						state.predictionResult.suggestion = sug ? marked(sug, { async: false }) : '';
						state.data = transformData(state.predictionResult);
						ElMessage.success('检测完成！');
						void persistImgRecord(payload);
					} catch {
		...
		.catch((err: unknown) => {
			...
			const msg =
				(err as any)?.message === 'Network Error'
					? '网络异常，请检查接口与跨域配置'
					: (err as any)?.message || '检测请求失败';
			ElMessage.error(msg);
		})
		.finally(() => {
			// 仅结束「当前这一次」请求的 loading，避免旧请求 finally 关掉新请求的转圈
			if (seq === predictRenderSeq) state.loading = false;
		});
};
...
onDeactivated(clearPageLoading);
onBeforeUnmount(clearPageLoading);
</script>
