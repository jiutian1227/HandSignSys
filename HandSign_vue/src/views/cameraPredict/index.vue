...
/** 离开路由后仍为 false，避免异步帧请求/收尾回调改已卸载组件状态 */
let cameraPageAlive = true;
...
const sendOneFrame = async () => {
	if (!cameraPageAlive || !state.cameraisShow || sending) return;
...
onMounted(() => {
	cameraPageAlive = true;
	getData();
});

onUnmounted(() => {
	cameraPageAlive = false;
	void stop();
});
</script>
