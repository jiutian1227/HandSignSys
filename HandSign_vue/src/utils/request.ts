...
const FLASK_PREFIX = '/flask';
const SPRING_ORIGIN = normalizeOrigin((import.meta.env.VITE_SPRING_ORIGIN as string) || 'https://springboot.fanzengxie.icu');
const FLASK_ORIGIN = normalizeOrigin((import.meta.env.VITE_FLASK_ORIGIN as string) || 'https://flask.fanzengxie.icu');
...
service.interceptors.request.use(
	(config: AxiosRequestConfig) => {
		...
		// 用 .env 控制联调模式：
		// - VITE_USE_DEV_PROXY=true (仅开发环境)：保留 /api /flask 相对路径，走 Vite 代理
		// - 其他情况：前端直连后端域名（不依赖 Vite 代理）
		if (typeof config.url === 'string' && !/^https?:\/\//i.test(config.url)) {
			if (!USE_DEV_PROXY) {
				if (config.url === API_PREFIX || config.url.startsWith(`${API_PREFIX}/`)) {
					config.url = `${SPRING_ORIGIN}${config.url.replace(new RegExp(`^${API_PREFIX}`), '')}`;
					// 避免 axios 仍然把 baseURL 拼接到绝对地址上
					config.baseURL = '';
				}
				if (config.url === FLASK_PREFIX || config.url.startsWith(`${FLASK_PREFIX}/`)) {
					// 与 Vite 代理 rewrite 一致：/flask 前缀在 Flask 服务上不存在，需去掉
					const flaskPath = config.url.replace(new RegExp(`^${FLASK_PREFIX}`), '') || '/';
					config.url = `${FLASK_ORIGIN}${flaskPath}`;
					config.baseURL = '';
				}
			}
		}
